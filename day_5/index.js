...
function customParseData() {
    ...
    storageChart.forEach((row, index) => {
...
function solutionA() {
    ...
    const reducedInstructions = data.map((order) => {
        return order.split(" ")
...
function solutionB() {
    ...
    reducedInstructions.forEach((instructions) => {
        ...
        const amountToTransfer = instr[0] > storage[instr[1]].length ? storage[instr[1]].length : instr[0]
        storage[instr[2]].push(...storage[instr[1]].splice(storage[instr[1]].length - amountToTransfer));
    })
